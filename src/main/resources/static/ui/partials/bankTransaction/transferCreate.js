app.controller('transferCreateCtrl', ['SellerService', 'BankTransactionService', '$scope', '$rootScope', '$timeout', '$log', '$uibModalInstance',
    function (SellerService, BankTransactionService, $scope, $rootScope, $timeout, $log, $uibModalInstance) {
        ...
        $scope.findFromSellerBalance = function () {
            SellerService.findSellerBalance($scope.buffer.fromSeller.id).then(function (value) {
                $scope.buffer.fromSeller = value;
            });
        };

        $scope.findToSellerBalance = function () {
            SellerService.findSellerBalance($scope.buffer.toSeller.id).then(function (value) {
                $scope.buffer.toSeller = value;
            });
        };
