app.controller("menuCtrl", [
    ...
    function (CompanyService,
        ...
        $scope.printDraft = function (contract) {
            window.open('/report/draft/' + contract.id);
        };
        $scope.rowMenuContract = [
            ...
            },
            {
                html: '<div class="drop-menu">' +
                '<img src="/ui/img/' + $rootScope.iconSet + '/print.' + $rootScope.iconSetType + '" width="24" height="24">' +
                '<span>طباعة كمبيالة</span>' +
                '</div>',
                enabled: function () {
                    return true;
                },
                click: function ($itemScope, $event, value) {
                    $scope.printDraft($itemScope.contract);
                }
            }
        ];
            ...
            ModalProvider.openConfirmModel("العقود", "delete", "هل تود حذف الدفعة فعلاً؟").result.then(function (value) {
                if (value) {
                    ContractPaymentService.remove(contractPayment.id).then(function () {
                        var index = $scope.contractPayments.indexOf(contractPayment);
                        $scope.contractPayments.splice(index, 1);
                    });
                }
            });
        };
        $scope.printContractPayment = function (contractPayment) {
            window.open('/report/contractPayment/' + contractPayment.id);
        };
        $scope.rowMenuContractPayment = [
            {
                html: '<div class="drop-menu">' +
                '<img src="/ui/img/' + $rootScope.iconSet + '/add.' + $rootScope.iconSetType + '" width="24" height="24">' +
                '<span>جديد...</span>' +
            ...
            },
            {
                ...
                '</div>',
                enabled: function () {
                    return $rootScope.contains($rootScope.me.team.authorities, ['ROLE_CONTRACT_PAYMENT_DELETE']);
                },
                click: function ($itemScope, $event, value) {
                    $scope.deleteContractPayment($itemScope.contractPayment);
                }
            },
            {
                html: '<div class="drop-menu">' +
                ...
                click: function ($itemScope, $event, value) {
                    $scope.printContractPayment($itemScope.contractPayment);
                }
            }
        ];
        ...
        $scope.printReceiptDeposit = function (bankTransaction) {
            window.open('/report/receiptDeposit/' + bankTransaction.id);
        };
        $scope.printReceiptWithdraw = function (bankTransaction) {
            window.open('/report/receiptWithdraw/' + bankTransaction.id);
        };
