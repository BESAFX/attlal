...
public class ContractPaymentRest {
    ...
    public String create(@RequestBody ContractPayment contractPayment) {
        ...
        if (contractPayment.getAmount() > 0) {
            ...
            builder.append("، البيان: " + contractPayment.getNote());
            builder.append("، الباقي من مبلغ العقد حتى تاريخه: " + (contract.getRemain() - contractPayment.getAmount()));
            builder.append(" ريال سعودي");
            bankTransaction.setNote(builder.toString());

            contractPayment.setBankTransaction(bankTransactionService.save(bankTransaction));
            contractPayment.setNote(builder.toString());
    ...
    public void delete(@PathVariable Long id) {
        ...
        if (contractPayment != null) {
            ...
            Person caller = ((PersonAwareUserDetails) SecurityContextHolder.getContext().getAuthentication().getPrincipal()).getPerson();
            StringBuilder builder = new StringBuilder();
            builder.append("سحب سند القبض رقم ");
            builder.append(" ( " + contractPayment.getCode() + " ) ");
            builder.append("، قيمة السند / ");
            builder.append(contractPayment.getAmount());
